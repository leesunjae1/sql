...
from employees  --9를쓰면 자리에있는 숫자만 나오고 0을쓰면 없는자리에 0이 채워진다
...
-- <이름> earns <$,월급> monthly but wants <$,월급 x 3>. 로 조회하라
...
select first_name, last_name,
    nullif(length(first_name), length(last_name)) --이름이 같으면 null이 나옴
from employees;
...
-- 사원들의 이름, 입사일, 입사요일을 월요일부터 요일순으로 조회하라.
