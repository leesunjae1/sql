...
select last_name
from employees
where lower(last_name) = 'higgins'; --lower펑션과 비교한다
...
-- 위 query에서 ' '가 trim 됐음을 눈으로 확인 할수 있게 조회하라
select '|' || trim(' ' from ' Hello ') || '|' 
from dual;
...
--위 query에서, where 절을 like로 refactoring 하라
select employee_id, concat(first_name, last_name) name,
job_id, length(last_name), instr(last_name, 'a') "Contains 'a'?"
from employees
where job_id like '%PROG';

-- 이름이 J나 A나 M으로 시작하는 사원들의 이름, 이름의 글자수를 조회하라
...
--사원들의 이름, 월급, 15.5%인상된 월급 (New Salary, 정수), 인상액(Increase)을 조회하라
select last_name, salary, 
    round(salary * 0.155 + salary) "New Salary", 
